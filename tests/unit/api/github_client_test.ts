...
import { GitHubClient, GitHubAPIError } from "../../../src/core/api/mod.ts";
...
Deno.test("GitHubClient", async (t) => {
  ...
  await t.step("getRepo fetches repository details", async () => {
    ...
    mockFetch.mock(`https://api.github.com/repos/${testRepo.owner.login}/${testRepo.name}`, {
      status: 200,
      body: testRepo
    });
  ...
  await t.step("getRepo returns null for non-existent repositories", async () => {
    // Arrange
    const mockFetch = new MockFetch();
  ...
  await t.step("handles rate limiting", async () => {
    ...
    mockFetch.mockSequence("https://api.github.com/user/starred?page=1&per_page=30", [
